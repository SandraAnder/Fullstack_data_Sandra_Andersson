...
def read_data():
    daata_path = Path(__file__).parents[1] / 'data'
    df = pd.read_csv(daata_path / 'cleaned_yh_region.csv',
...
def layout():
    df = read_data()
    df_reset = df.reset_index(names=['year']).style.format(
        {'year': lambda x: f'{x}'})

    st.markdown('# YH dashboard')

    st.markdown(
        'This data shows number of started educations per region and year')
    ...
    fig = px.line(data_frame=df, x=df.index,
                  y=df[region], title=f'Started educations in {region} 2007-2023', labels={'index': 'year', region: 'started educations'})
